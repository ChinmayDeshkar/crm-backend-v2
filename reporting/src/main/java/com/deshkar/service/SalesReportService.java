...
public class SalesReportService {
    ...
    private final SalesRepo purchaseRepository;
    ...
    public void sendDailySalesReport() {
        ...
        List<Sales> todaysPurchases = purchaseRepository.findTodaysPurchases(startOfDay, endOfDay);
        ...
        double totalRevenue = todaysPurchases.stream()
                .mapToDouble(Sales::getAdvancePaid)
                .sum();

        double totalBalance = todaysPurchases.stream()
                .mapToDouble(Sales::getBalance)
                .sum();
        ...
        Map<String, List<Sales>> paymentGrouped = todaysPurchases.stream()
        ...
        for (String method : List.of("CASH", "CARD", "UPI")) {
            List<Sales> list = paymentGrouped.getOrDefault(method, List.of());
            double amount = list.stream().mapToDouble(Sales::getAdvancePaid).sum();
